...
Generate 5 practice questions that target the student's specific gaps. Vary question types (short answer, explain, give example). Base questions ONLY on the NCERT content provided.`;
  ...
  app.post("/api/homework", authMiddleware, async (req: AuthRequest, res) => {
    ...
    try {
      const { subject, className, section, instruction, modelSolutionText, dueDate } = req.body;
      if (!subject || !className || !section || !instruction || !dueDate) {
      ...
      const hw = await storage.createHomework({
        ...
        section,
        instruction,
        modelSolutionText: modelSolutionText || null,
        dueDate,
      });
  ...
  // Teacher gets their own homework list
  app.get("/api/teacher/homework", authMiddleware, async (req: AuthRequest, res) => {
    if (req.user?.role !== "teacher") return res.status(403).json({ message: "Unauthorized" });
    try {
      const hwList = await storage.getHomeworkByTeacher(req.user.id);
      // Attach submission counts
      const result = await Promise.all(hwList.map(async (hw) => {
        const submissions = await storage.getHomeworkSubmissionsByHomework(hw.id);
        return {
          ...hw,
  ...
  // Teacher gets submissions for a specific homework
  app.get("/api/homework/:id/submissions", authMiddleware, async (req: AuthRequest, res) => {
    if (req.user?.role !== "teacher") return res.status(403).json({ message: "Unauthorized" });
    try {
      const hw = await storage.getHomework(parseInt(req.params.id));
      if (!hw || hw.teacherId !== req.user.id) return res.status(403).json({ message: "Access denied" });
      const submissions = await storage.getHomeworkSubmissionsByHomework(hw.id);
  ...
  // Student gets homework for their class/section (auto-synced)
  app.get("/api/student/homework", authMiddleware, async (req: AuthRequest, res) => {
    ...
    try {
      ...
      const hwList = await storage.getHomeworkByClassSection(student.studentClass, student.section);

      // Attach submission status for each homework
      const result = await Promise.all(hwList.map(async (hw) => {
        const submission = await storage.getHomeworkSubmission(hw.id, req.user!.id);
        return {
          ...hw,
  ...
  // Student submits homework (image upload → OCR → AI evaluation)
  app.post("/api/student/homework/:id/submit", authMiddleware, async (req: AuthRequest, res) => {
    ...
    try {
      ...
      if (hw.className !== student.studentClass || hw.section !== student.section) {
      ...
      // OCR extraction from handwritten homework
      console.log(`[HW-OCR] Extracting text from student homework submission...`);
      const ocrResponse = await getOpenAIClient().chat.completions.create({
        model: "gpt-4o",
        messages: [{
          role: "user",
          content: [
            { type: "text", text: "Extract all the handwritten text from this homework submission image. Return only the extracted text, no commentary." },
            { type: "image_url", image_url: { url: imageBase64 } }
          ]
        }]
      });
      ...
      // AI evaluation against model solution
      let score: number | null = null;
      let evalStatus = "completed";

      if (hw.modelSolutionText && ocrText) {
        console.log(`[HW-EVAL] Evaluating homework against model solution...`);
        const evalResponse = await getOpenAIClient().chat.completions.create({
          model: "gpt-4o",
          messages: [{
            role: "user",
            content: `You are evaluating a student's homework submission.

Homework Instruction: ${hw.instruction}

Model Solution: ${hw.modelSolutionText}

Student's Submission (OCR extracted): ${ocrText}

Evaluate the student's work on a scale of 0-100. Consider accuracy, completeness, and understanding.
Return ONLY valid JSON: {"score": <number 0-100>, "feedback": "<brief 1-2 sentence feedback>", "status": "<completed|needs_improvement>"}`
          }],
          response_format: { type: "json_object" },
        });
        try {
          ...
          evalStatus = evalData.status === "needs_improvement" ? "needs_improvement" : "completed";
        } catch {
          score = 70;
        }
      } else if (ocrText) {
      ...
      // Check for existing submission (update) or create new
      const existing = await storage.getHomeworkSubmission(hw.id, req.user.id);
      if (existing) {
        const updated = await storage.updateHomeworkSubmission(existing.id, {
          ocrText,
          score,
          status: evalStatus,
      ...
      res.status(500).json({ message: "Failed to submit homework", detail: err?.message });
    }
  });

  // Student homework regularity score
  app.get("/api/student/homework-regularity", authMiddleware, async (req: AuthRequest, res) => {
    if (req.user?.role !== "student") return res.status(403).json({ message: "Unauthorized" });
    try {
      const student = await storage.getStudent(req.user.id);
      if (!student) return res.status(404).json({ message: "Student not found" });
      ...
      const hwList = await storage.getHomeworkByClassSection(student.studentClass, student.section);
      const assigned = hwList.length;

      if (assigned === 0) {
        return res.json({ assigned: 0, submitted: 0, score: 0, label: "No homework assigned", rating: "Regular" });
      }
      ...
      const submitted = hwList.filter(hw => submittedIds.has(hw.id)).length;
      const score = Math.round((submitted / assigned) * 100);
      const rating = score >= 75 ? "Regular" : score >= 40 ? "Moderate" : "Irregular";

      res.json({ assigned, submitted, score, rating });
    } catch (err: any) {
      res.status(500).json({ message: "Failed to calculate regularity", detail: err?.message });
    }
  });

  // Teacher homework analytics via conversational AI
  app.post("/api/teacher/homework-analytics", authMiddleware, async (req: AuthRequest, res) => {
    if (req.user?.role !== "teacher") return res.status(403).json({ message: "Unauthorized" });
    try {
      ...
      const hwList = await storage.getHomeworkByTeacher(req.user.id);
      const hwWithSubmissions = await Promise.all(hwList.map(async (hw) => {
        const submissions = await storage.getHomeworkSubmissionsByHomework(hw.id);
        return {
          id: hw.id,
          subject: hw.subject,
          ...
          section: hw.section,
          instruction: hw.instruction.substring(0, 100),
          dueDate: hw.dueDate,
          ...
          avgScore: submissions.length > 0
            ? Math.round(submissions.filter(s => s.score !== null).reduce((a, s) => a + (s.score ?? 0), 0) / Math.max(submissions.filter(s => s.score !== null).length, 1))
            : null,
      ...
      const context = JSON.stringify(hwWithSubmissions, null, 2);
      ...
      const response = await getOpenAIClient().chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: `You are a school analytics assistant for a teacher. Use the homework data below to answer questions accurately.
Be concise and specific. Use bullet points when listing students.
Homework data: ${context}`,
          },
          { role: "user", content: query },
        ],
      });

      const answer = response.choices[0].message.content || "Could not process query.";
      res.json({ answer, query });
    } catch (err: any) {
      res.status(500).json({ message: "Homework analytics failed", detail: err?.message });
    }
  });
